
import { Template, CardFont, Pattern } from './types';

export const FLAT_ICONS: Record<string, string> = {
  phone: "M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z",
  email: "M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z",
  website: "M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9",
  address: "M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z M15 11a3 3 0 11-6 0 3 3 0 016 0z",
  whatsapp: "M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z",
  facebook: "M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z",
  linkedin: "M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z",
  instagram: "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z",
  twitter: "M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z",
  info: "M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z",
  alignLeft: "M4 6h16M4 12h10M4 18h16",
  alignCenter: "M4 6h16M7 12h10M4 18h16",
  alignRight: "M4 6h16M10 12h10M4 18h16"
};

export const TEMPLATES: Template[] = [
  { id: 'modern', name: 'عصري', description: 'تصميم يواكب أحدث الصيحات' },
  { id: 'classic', name: 'كلاسيكي', description: 'رسمية وهدوء تام' },
  { id: 'creative', name: 'إبداعي', description: 'لمسات فنية غير تقليدية' },
  { id: 'minimal', name: 'بسيط', description: 'الجمال في البساطة' },
  { id: 'elegant', name: 'أنيق', description: 'فخامة بتفاصيل رقيقة' },
  { id: 'tech', name: 'تقني', description: 'للمتخصصين في التكنولوجيا' },
  { id: 'corporate', name: 'شركات', description: 'رسمي جداً للمنشآت' },
  { id: 'startup', name: 'ناشئة', description: 'حيوية وطاقة عالية' },
  { id: 'geometric', name: 'هندسي', description: 'توازن رياضي دقيق' },
  { id: 'mini-plus', name: 'ميني بلس', description: 'تركيز عالي على المحتوى' },
  { id: 'gradient', name: 'تدرج', description: 'ألوان متداخلة بانسيابية' },
  { id: 'linear', name: 'خطي', description: 'خطوط هيكلية واضحة' },
  { id: 'luxury', name: 'فاخر', description: 'لمسات ملكية ذهبية' },
  { id: 'swiss', name: 'سويسري', description: 'نظام ودقة متناهية' },
  { id: 'abstract', name: 'تجريدي', description: 'فن خالص يعبر عنك' },
  { id: 'grid-layout', name: 'شبكي', description: 'توزيع منظم جداً' },
  { id: 'signature', name: 'علامة بارزة', description: 'اسمك هو البطل' },
  { id: 'circular', name: 'دائري', description: 'انحناءات ناعمة ومريحة' },
  { id: 'lines-layout', name: 'خطوط', description: 'بساطة القلم والورقة' },
  { id: 'glass', name: 'زجاجي', description: 'تأثير الشفافية الحديث' },
  { id: 'neon', name: 'نيون', description: 'إضاءة قوية وجذابة' },
  { id: 'x-layout', name: 'اكس', description: 'هيكل متقاطع فريد' }
];

export const PATTERNS: Pattern[] = [
  { id: 'none', name: 'بدون نمط', nameEn: 'None' },
  { id: 'dots', name: 'نقاط هندسية', nameEn: 'Dots' },
  { id: 'grid', name: 'شبكة العمل', nameEn: 'Grid' },
  { id: 'stripes', name: 'خطوط رسمية', nameEn: 'Stripes' },
  { id: 'topography', name: 'منحنيات عصرية', nameEn: 'Curves' },
  { id: 'polygons', name: 'مضلعات خفيفة', nameEn: 'Polygons' },
  { id: 'circuit', name: 'مسارات تقنية', nameEn: 'Circuit' },
  { id: 'bubbles', name: 'هالة احترافية', nameEn: 'Halos' },
  { id: 'bauhaus', name: 'فن تجريدي', nameEn: 'Bauhaus' },
  { id: 'waves', name: 'أمواج هادئة', nameEn: 'Waves' },
  { id: 'blueprint', name: 'مخطط هندسي', nameEn: 'Blueprint' },
  { id: 'stellar', name: 'نجوم خافتة', nameEn: 'Stellar' },
  { id: 'mosaic', name: 'فسيفساء عصرية', nameEn: 'Mosaic' },
  { id: 'abstract', name: 'تجريد فني', nameEn: 'Abstract' },
  { id: 'prism', name: 'تكسر الضوء', nameEn: 'Prism' }
];

export const ARABIC_FONTS: CardFont[] = [
  // الخطوط الرسمية والحديثة
  { id: 'cairo', name: 'كايرو (Cairo) - افتراضي', family: "'Cairo', sans-serif" },
  { id: 'tajawal', name: 'تجوال (Tajawal) - رسمي', family: "'Tajawal', sans-serif" },
  { id: 'almarai', name: 'المراعي (Almarai) - ناعم', family: "'Almarai', sans-serif" },
  
  // الخطوط الكرتونية والإبداعية (Cartoon & Fun)
  { id: 'lalezar', name: 'لاليزار (Lalezar) - كرتوني ضخم', family: "'Lalezar', cursive" },
  { id: 'jomhuria', name: 'جمهورية (Jomhuria) - كرتوني عريض', family: "'Jomhuria', serif" },
  { id: 'changa', name: 'شانغا (Changa) - هندسي مرح', family: "'Changa', sans-serif" },
  { id: 'lemonada', name: 'ليمونادة (Lemonada) - دائري ممتع', family: "'Lemonada', cursive" },
  { id: 'rakkas', name: 'رقاص (Rakkas) - إبداعي جريء', family: "'Rakkas', cursive" },
  { id: 'vibes', name: 'فايبز (Vibes) - فني متصل', family: "'Vibes', cursive" },
  
  // الخطوط الكلاسيكية
  { id: 'amiri', name: 'الأميري (Amiri) - كلاسيكي', family: "'Amiri', serif" }
];

export const INITIAL_CARD_DATA_AR = {
  name: 'عبدالرحمن العلي',
  title: 'مستشار تطوير الأعمال',
  company: 'مجموعة الرؤية الاستثمارية',
  phone: '+966 55 000 0000',
  email: 'a.ali@vision.sa',
  website: 'www.vision.sa',
  address: 'حي النخيل، طريق الملك فهد، الرياض',
  primaryColor: '#0f172a',
  secondaryColor: '#3b82f6',
  frontBackgroundColor: '#ffffff',
  backBackgroundColor: '#0f172a',
  frontCustomBgUrl: null,
  backCustomBgUrl: null,
  textColor: '#1e293b',
  layout: 'modern' as const,
  pattern: 'none' as const,
  fontFamily: "'Cairo', sans-serif",
  logoText: 'VISION',
  tagline: 'نصنع المستقبل بإتقان',
  autoTextColor: true,
  logoUrl: null,
  frontLogoScale: 0.6,
  frontLogoX: 50,
  frontLogoY: 20,
  frontLogoVisible: true,
  frontLogoWhite: false,
  backLogoScale: 1.5,
  backLogoX: 50,
  backLogoY: 45,
  backLogoVisible: true,
  backLogoWhite: true,
  logoBgRemoved: false,
  // Text Positions & Align
  frontNameX: 50,
  frontNameY: 45,
  frontNameAlign: 'center' as const,
  frontContactX: 50,
  frontContactY: 75,
  frontContactAlign: 'center' as const,
  backCompanyX: 50,
  backCompanyY: 80,
  backCompanyAlign: 'center' as const,
  // Font Sizes
  nameFontSize: 32,
  titleFontSize: 16,
  contactFontSize: 10,
  companyFontSize: 28,
  taglineFontSize: 11,
  icons: {
    phone: 'phone',
    email: 'email',
    website: 'website',
    address: 'address'
  },
  extraFields: []
};

export const INITIAL_CARD_DATA_EN = {
  name: 'Abdulrahman Al-Ali',
  title: 'Business Development Consultant',
  company: 'Vision Investment Group',
  phone: '+966 55 000 0000',
  email: 'a.ali@vision.sa',
  website: 'www.vision.sa',
  address: 'An-Nakheel, King Fahd Rd, Riyadh',
  primaryColor: '#0f172a',
  secondaryColor: '#3b82f6',
  frontBackgroundColor: '#ffffff',
  backBackgroundColor: '#0f172a',
  frontCustomBgUrl: null,
  backCustomBgUrl: null,
  textColor: '#1e293b',
  layout: 'modern' as const,
  pattern: 'none' as const,
  fontFamily: "'Cairo', sans-serif",
  logoText: 'VISION',
  tagline: 'Crafting the future with excellence',
  autoTextColor: true,
  logoUrl: null,
  frontLogoScale: 0.6,
  frontLogoX: 50,
  frontLogoY: 20,
  frontLogoVisible: true,
  frontLogoWhite: false,
  backLogoScale: 1.5,
  backLogoX: 50,
  backLogoY: 45,
  backLogoVisible: true,
  backLogoWhite: true,
  logoBgRemoved: false,
  // Text Positions & Align
  frontNameX: 50,
  frontNameY: 45,
  frontNameAlign: 'center' as const,
  frontContactX: 50,
  frontContactY: 75,
  frontContactAlign: 'center' as const,
  backCompanyX: 50,
  backCompanyY: 80,
  backCompanyAlign: 'center' as const,
  // Font Sizes
  nameFontSize: 32,
  titleFontSize: 16,
  contactFontSize: 10,
  companyFontSize: 28,
  taglineFontSize: 11,
  icons: {
    phone: 'phone',
    email: 'email',
    website: 'website',
    address: 'address'
  },
  extraFields: []
};

export const TRANSLATIONS = {
  ar: {
    appName: "برو كارد",
    designHub: "محرك التصميم",
    brandingSuite: "الهوية الاحترافية",
    previewHighRes: "تحريك العناصر متاح",
    sideA: "الوجه الرئيسي",
    sideB: "الخلفية",
    frontSide: "الأمامي",
    backSide: "الخلفي",
    exportEngineMsg: "كرت العمل الخاص بك جاهز للطباعة بدقة 300DPI. يمكنك سحب العناصر لتغيير مكانها.",
    allRightsReserved: "جميع الحقوق محفوظة 2025",
    layout: "القالب",
    bgPattern: "نمط الخلفية",
    font: "الخط",
    logoManagement: "الشعار",
    uploadLogo: "رفع شعار",
    updateLogo: "تغيير الشعار",
    removeBg: "إزالة الخلفية",
    show: "عرض",
    white: "أبيض",
    logoSize: "الحجم",
    fontSize: "حجم الخط",
    alignment: "المحاذاة",
    horizontal: "X",
    vertical: "Y",
    colorPanel: "الألوان",
    smartContrast: "تباين تلقائي",
    bgFront: "خلفية A",
    bgBack: "خلفية B",
    personalData: "المعلومات",
    contactData: "التواصل",
    fullName: "الاسم",
    jobTitle: "المنصب",
    phone: "الهاتف",
    email: "البريد",
    website: "الموقع",
    address: "العنوان",
    extraFields: "إضافات",
    addLine: "إضافة حقل",
    noExtraFields: "لا توجد حقول",
    corporateIdentity: "المنشأة",
    companyName: "الشركة",
    tagline: "الشعار اللفظي",
    downloadPDF: "تحميل PDF للطباعة",
    downloadPNG: "حفظ صورة PNG",
    downloadSVG: "تصدير SVG",
    langToggle: "English",
    customBg: "خلفية خاصة",
    uploadFrontBg: "رفع خلفية للأمام",
    uploadBackBg: "رفع خلفية للخلف",
    removeCustomBg: "إزالة الخلفية"
  },
  en: {
    appName: "Pro Card",
    designHub: "Design Hub",
    brandingSuite: "Branding Suite",
    previewHighRes: "Drag and Drop Active",
    sideA: "Front",
    sideB: "Back",
    frontSide: "Front Side",
    backSide: "Back Side",
    exportEngineMsg: "Your card is ready for 300DPI professional print. Drag elements to reposition.",
    allRightsReserved: "All Rights Reserved 2025",
    layout: "Layout",
    bgPattern: "Background Pattern",
    font: "Font",
    logoManagement: "Logo",
    uploadLogo: "Upload Logo",
    updateLogo: "Update Logo",
    removeBg: "Remove BG",
    show: "Show",
    white: "White",
    logoSize: "Size",
    fontSize: "Font Size",
    alignment: "Alignment",
    horizontal: "X",
    vertical: "Y",
    colorPanel: "Colors",
    smartContrast: "Auto",
    bgFront: "Front BG",
    bgBack: "Back BG",
    personalData: "Info",
    contactData: "Contact",
    fullName: "Name",
    jobTitle: "Title",
    phone: "Phone",
    email: "Email",
    website: "Web",
    address: "Address",
    extraFields: "Extra",
    addLine: "Add Field",
    noExtraFields: "No extra fields",
    corporateIdentity: "Identity",
    companyName: "Company",
    tagline: "Tagline",
    downloadPDF: "Print PDF",
    downloadPNG: "Save PNG",
    downloadSVG: "Export SVG",
    langToggle: "العربية",
    customBg: "Custom BG",
    uploadFrontBg: "Front BG",
    uploadBackBg: "Back BG",
    removeCustomBg: "Remove"
  }
};
